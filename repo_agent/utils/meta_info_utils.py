...
def make_fake_files():
    ...
    repo = git.Repo(setting.project.target_repo)  # type: ignore
    ...
    for diff_file in itertools.chain(
        ...
        if now_file_path.endswith((".py", ".java", ".go", ".kt", ".kts")):
            ...
            if os.path.exists(os.path.join(setting.project.target_repo, now_file_path)):  # type: ignore
                os.rename(
                    os.path.join(setting.project.target_repo, now_file_path),  # type: ignore
                    os.path.join(setting.project.target_repo, latest_file_path),  # type: ignore
                )
                ...
                with open(
                    os.path.join(setting.project.target_repo, latest_file_path), "w"  # type: ignore
                ) as writer:
                    pass
            with open(
                os.path.join(setting.project.target_repo, now_file_path), "w"  # type: ignore
            ) as writer:
        ...
        for fi in files:
            ...
            if os.path.isdir(fi_d):
                ...
                if os.path.getsize(fi_d) == 0:
                    print(
                        f"{Fore.LIGHTRED_EX}[Deleting Temp File]: {Style.RESET_ALL}{fi_d[len(str(setting.project.target_repo)):]}, {origin_name[len(str(setting.project.target_repo)):]}"  # type: ignore
                    )  # type: ignore
                    os.remove(fi_d)
                else:
                    print(
                        f"{Fore.LIGHTRED_EX}[Recovering Latest Version]: {Style.RESET_ALL}{origin_name[len(str(setting.project.target_repo)):]} <- {fi_d[len(str(setting.project.target_repo)):]}"  # type: ignore
                    )  # type: ignore
                    os.rename(fi_d, origin_name)

    gci(setting.project.target_repo)  # type: ignore
