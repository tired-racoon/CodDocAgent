...
class ProjectHandler:
    ...
    def get_supported_files(self) -> List[str]:
        ...
        for root, dirs, files in os.walk(self.repo_path):
            dirs[:] = [
                ...
                for d in dirs
                if not d.startswith(".")
                and d not in {"node_modules", "target", "build", "dist", "__pycache__"}
            ]
    ...
    def _get_class_methods(self, structure: List[Dict], class_name: str) -> List[Dict]:
        ...
        for item in structure:
            if item.get("parent") == class_name and item["type"] in [
                "FunctionDef",
                "AsyncFunctionDef",
                "Function",
                "method_declaration",
    ...
    def find_definition(
        ...
        try:
            references = self.reference_finder.get_references(
                file_path, line, column, scope="project"
            )
    ...
    def get_references(
        self, file_path: str, line: int, column: int, scope: str = "project"
...
"""
    ...
    def find_symbol_references(
        self, file_path: str, line: int, column: int, in_file_only: bool = False
    ) -> List[Tuple[str, int, int]]:
