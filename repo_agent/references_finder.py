...
class ReferenceFinder:
    def __init__(self, repo_path: str, file_path: str = ""):
        ...
        self.import_patterns = {
            ...
            "java": {"import_declaration": ["name"]},
            "go": {"import_declaration": ["source"]},
    ...
    def _get_extension(self, language: str) -> str:
        ...
        extensions = {"python": "py", "java": "java", "go": "go", "kotlin": "kt"}
    ...
    def _find_node_at_position(
        self, root: Node, line: int, column: int
    ) -> Optional[Node]:
    ...
    def _get_identifier_at_position(
        self, root: Node, code: bytes, language: str, line: int, column: int
    ) -> Optional[str]:
        ...
        while current:
            if current.type in identifier_types:
                ...
                if (
                    current.start_point[0] <= line - 1 <= current.end_point[0]
                    and current.start_point[1] <= column <= current.end_point[1]
                ):
    ...
    def _find_all_identifiers(
        self, root: Node, code: bytes, language: str, target_name: str
        ...
        def walk(node: Node):
            if node.type in identifier_types:
                ...
                if text == target_name:
                    ...
                    matches.append(
                        (node.start_point[0] + 1, node.start_point[1], context)
                    )
        ...
        while current:
            ...
            if language == "python":
                ...
                if (
                    ...
                    and current.child_by_field_name("left")
                    and self._node_contains_position(
                        current.child_by_field_name("left"), node
                    )
                ):
                    return "definition"
                ...
                if (
                    ...
                    and current.child_by_field_name("name") == node
                ):
                    return "definition"
            elif language == "java":
                if (
    ...
    def _is_same_scope(
        self, root: Node, language: str, pos1: Tuple[int, int], pos2: Tuple[int, int]
    ) -> bool:
    ...
    def _filter_references_by_scope(
        self,
    ...
    def find_references_in_file(
        ...
        try:
            ...
            matches = self._find_all_identifiers(
                root, code_bytes, language, variable_name
            )
    ...
    def _extract_imports(
        self, root: Node, code: bytes, language: str
    ) -> Dict[str, Set[str]]:
            ...
            if node.type in patterns:
                if language == "python":
                    if node.type == "import_statement":
                        ...
                        for child in node.children:
                            if (
                                child.type == "dotted_as_names"
                                or child.type == "dotted_name"
                            ):
                                for name_node in child.children:
                                    if (
                                        name_node.type == "dotted_name"
                        ...
                        for child in node.children:
                            if child.type == "import_list":
                                for name_child in child.children:
                                    if (
                                        name_child.type == "dotted_name"
                                        or name_child.type == "identifier"
                                    ):
                                        imported_name = self._get_node_text(
                                            name_child, code
                                        )
                                        imports["from_imports"].add(f"{module_name}.{imported_name}")  # type: ignore
                    ...
                    if node.type == "import_declaration":
                        ...
                        # Remove 'import ' and ';'
                        import_path = (
                            import_path.replace("import ", "").replace(";", "").strip()
                        )
                    ...
                    if node.type == "import_declaration":
                        for child in node.children:
                            if child.type == "import_spec":
                                ...
                                if source_node:
                                    import_path = self._get_node_text(
                                        source_node, code
                                    ).strip('"')
                                    imports["imports"].add(import_path)
    ...
    def _get_function_full_path(
        self, file_path: str, function_name: str, class_name: str = ""
    ) -> str:
    ...
    def _find_function_context(
        self, root: Node, code: bytes, language: str, target_line: int, target_col: int
    ...
    def _filter_references_by_imports(
        self, references: List[Tuple[str, int, int, str]], origin_file: str
    ) -> List[Tuple[str, int, int, str]]:
                ...
                if ref_file == origin_rel_path:
                    for ref_line, ref_col, ref_name in file_refs:
                        filtered_references.append(
                            (ref_file, ref_line, ref_col, ref_name)
                        )
                    continue
                    ...
                    for ref_line, ref_col, ref_name in file_refs:
                        ...
                        if class_name and func_name:
                            potential_paths.append(
                                self._get_function_full_path(
                                    abs_ref_path, func_name, class_name
                                )
                            )
                        if func_name:
                            potential_paths.append(
                                self._get_function_full_path(abs_ref_path, func_name)
                            )

                        potential_paths.append(
                            self._get_function_full_path(abs_ref_path, ref_name)
                        )
    ...
    def _check_local_definition(
        ...
        if language == "python":
            return self._check_python_local_definition(
                root, code, target_name, target_line, target_col
            )
        elif language == "java":
            return self._check_java_local_definition(
                root, code, target_name, target_line, target_col
            )
        elif language == "go":
            return self._check_go_local_definition(
                root, code, target_name, target_line, target_col
            )
        elif language == "kotlin":
            return self._check_kotlin_local_definition(
                root, code, target_name, target_line, target_col
            )
        return False

    def _check_python_local_definition(
        self,
        root: Node,
        code: bytes,
        target_name: str,
    ...
    def _check_java_local_definition(
        self,
        root: Node,
        code: bytes,
        target_name: str,
    ...
    def _check_go_local_definition(
        self,
        root: Node,
        code: bytes,
        target_name: str,
    ...
    def _check_kotlin_local_definition(
        self,
        root: Node,
        code: bytes,
        target_name: str,
    ...
    def find_references_in_repo(
        self, variable_name: str, origin_file: str, origin_line: int, origin_column: int
    ) -> List[Tuple[str, int, int, str]]:
        ...
        filtered_references = self._filter_references_by_imports(
            all_references, origin_file
        )

        return filtered_references

    def get_references(
        self, line: int, column: int, scope: str = "project"
        ...
        try:
            ...
            if not variable_name:
                logger.warning(
                    f"No identifier found at {self.file_path}:{line}:{column}"
                )
                return []

            logger.debug(
                f"Found identifier '{variable_name}' at {self.file_path}:{line}:{column}"
            )
            ...
            logger.error(f"Parameters: line={line}, column={column}, scope={scope}")
            return []
...
def find_all_referencer(
    ...
    try:
        ...
        filtered_refs = [
            ...
            for ref_file, ref_line, ref_col in references  # type: ignore
            if not (
                ref_file == file_path
                and ref_line == line_number
                and ref_col == column_number
            )
        ]
